...
export const AnimatedTooltip = ({ items }: AnimatedTooltipProps) => {
  ...
  return (
    <div className="relative z-[100] flex flex-row items-center">
      {items.map((item) => (
        <div
          className="group relative -mr-4 z-[100]"
          key={item.name}
          ...
          <AnimatePresence>
            {hoveredIndex === item.id && (
              <motion.div
                ...
                className="absolute -top-16 left-1/2 z-[200] flex -translate-x-1/2 flex-col items-center justify-center rounded-md bg-black px-4 py-2 text-xs shadow-xl"
              >
